...
def generate_video(args):
    ...
    # Validate video_length: must be 121-721 frames (5-30 sec) and satisfy (N-1) % 4 == 0
    if args.video_length < 121 or args.video_length > 721:
        raise ValueError(
            f"video_length must be between 121 (5 sec) and 721 (30 sec), got {args.video_length}."
        )
    if (args.video_length - 1) % 4 != 0:
        raise ValueError(
            f"video_length must satisfy (N-1) % 4 == 0 for temporal compression. "
            f"Got {args.video_length}. Valid examples: 121, 241, 361, 481, 601, 721."
        )
...
def main():
    ...
    parser.add_argument(
        '--resolution', type=str, default='480p', choices=['480p'],
        help='Video resolution (locked to 480p for optimized memory usage)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Number of frames to generate (default: 121, ~5 sec). '
             'Valid range: 121 (5 sec) to 721 (30 sec). Must satisfy (N-1) %% 4 == 0.'
    )
    parser.add_argument(
        '--sr', type=str_to_bool, nargs='?', const=True, default=False,
        help='Enable super resolution (default: false, disabled to save GPU memory). '
    ...
    parser.add_argument(
        '--enable_cache', type=str_to_bool, nargs='?', const=True, default=True,
        help='Enable cache for transformer (default: true). '
